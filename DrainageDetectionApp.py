...
# -------------------------------------------------
# CSS: Smart city dark + soft glows + animated drift + icons
# -------------------------------------------------
st.markdown(
    """
<style>
...
.header {
  ...
  box-shadow: 0 0 35px rgba(91,192,255,0.10), 0 18px 40px rgba(0,0,0,0.45);
}
...
.card, .kpi {
  ...
  box-shadow: 0 0 25px rgba(91,192,255,0.05), 0 14px 30px rgba(0,0,0,0.40);
...
.card:hover, .kpi:hover {
  ...
  box-shadow: 0 0 45px rgba(91,192,255,0.18), 0 18px 45px rgba(0,0,0,0.50);
}
...
.kpi-value {
  ...
  text-shadow: 0 0 12px rgba(91,192,255,0.35), 0 0 20px rgba(91,192,255,0.15);
}
...
.pill {
  ...
  box-shadow: 0 0 10px rgba(91,192,255,0.12);
  margin-left: 8px;
}
.pill-ok   { background: rgba(16,185,129,0.15); }
.pill-warn { background: rgba(245,158,11,0.16); }
.pill-crit { background: rgba(239,68,68,0.16); }
.pill-info { background: rgba(59,130,246,0.16); }

/* progress glow */
div[data-testid="stProgressBar"] > div > div {
  background: linear-gradient(90deg, #5BC0FF, #38A9F8) !important;
  box-shadow: 0 0 14px rgba(91,192,255,0.8), 0 0 25px rgba(91,192,255,0.4);
}
...
section[data-testid="stSidebar"] * { color: #e6eefc; }
...
[data-testid="stDataFrame"] {
  ...
  overflow: hidden;
  border: 1px solid rgba(91,192,255,0.15);
  box-shadow: 0 0 25px rgba(91,192,255,0.08), 0 10px 30px rgba(0,0,0,0.35);
}
...
def section_title(icon_class: str, title: str) -> str:
    return f"<div class='section-title'><i class='fa-solid {icon_class}'></i><span>{title}</span></div>"
...
# -------------------------------------------------
# DATA + MODEL (design-feature based)
# -------------------------------------------------
def make_demo_data(n_hours: int = 24) -> pd.DataFrame:
...
def build_zone_table(risk_pack: dict, clog_pct: float) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows, columns=["zone", "risk_score"]).sort_values("risk_score", ascending=False).reset_index(drop=True)
...
# -------------------------------------------------
# AUTO-REFRESH (built-in, non-blocking)
# -------------------------------------------------
...
<div class="header">
  <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:14px; flex-wrap:wrap;">
    ...
    <div style="white-space:nowrap;">
      {pill_html("SYSTEM: " + severity, sev_cls)}
      {pill_html("Last update: " + pd.to_datetime(latest["timestamp"]).strftime("%Y-%m-%d %H:%M"), "pill-info")}
    </div>
  </div>
</div>
"""
...
c1.markdown(kpi_box("fa-cloud-rain", "Rain intensity (mm/h)", f"{rain_now:.2f}"), unsafe_allow_html=True)
c2.markdown(kpi_box("fa-water", "Drain flow (L/s)", f"{flow_now:.2f}"), unsafe_allow_html=True)
c3.markdown(kpi_box("fa-tank-water", "Tank fill (%)", f"{tank_now:.1f}"), unsafe_allow_html=True)
...
with left:
    st.markdown(f"<div class='card'>{section_title('fa-chart-line', 'Trend: Rain vs Tank Level')}</div>", unsafe_allow_html=True)
    fig = px.line(
        df,
        x="timestamp",
    ...
    fig.update_layout(template="plotly_dark", height=420, margin=dict(l=10, r=10, t=35, b=10))
    st.plotly_chart(fig, use_container_width=True)

    st.markdown(f"<div class='card'>{section_title('fa-gauge', 'Flood Risk Indicator')}</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div class='card'>{section_title('fa-database', 'Raw Sensor Feed')}</div>", unsafe_allow_html=True)
    st.dataframe(df.tail(min(len(df), 24)), use_container_width=True, hide_index=True)

with right:
    st.markdown(f"<div class='card'>{section_title('fa-bell', 'Alerts')}</div>", unsafe_allow_html=True)
    alerts = []
    if tank_critical:
        alerts.append(("Tank level high", f"Tank at {tank_now:.1f}% (threshold {tank_crit}%)"))
    if flow_blocked:
        alerts.append(("Flow restriction detected", f"Flow at {flow_now:.2f} L/s (threshold {flow_block_thresh:.1f})"))
    if rain_spike:
        alerts.append(("Rain spike detected", f"Increase: {rain_now - rain_prev:.2f} mm/h (threshold {rain_spike_delta:.1f})"))
    if risk_now >= 80:
    ...
    st.markdown(f"<div class='card'>{section_title('fa-layer-group', 'Zone Risk Levels')}</div>", unsafe_allow_html=True)
    ...
    for _, r in dfz.iterrows():
        ...
        st.progress(int(r["risk_score"]), text=f"{tag} · {r['zone']} — {int(r['risk_score'])}/100")

    st.dataframe(dfz[["zone", "risk_score", "status"]], use_container_width=True, hide_index=True)

    st.markdown(f"<div class='card'>{section_title('fa-diagram-project', 'Model Breakdown')}</div>", unsafe_allow_html=True)
    m1, m2 = st.columns(2)
